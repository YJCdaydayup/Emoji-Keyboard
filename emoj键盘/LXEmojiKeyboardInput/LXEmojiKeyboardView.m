...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    //新增ScrollView
    self.addButtonScrollView = [[UIScrollView alloc]init];
    self.addButtonScrollView.backgroundColor = [UIColor cyanColor];
    self.addButtonScrollView.pagingEnabled = YES;
    self.addButtonScrollView.contentSize = CGSizeMake(2*self.frame.size.width, self.frame.size.height);
    self.addButtonScrollView.showsHorizontalScrollIndicator = YES;
    
    [self addSubview:self.addButtonScrollView];
    
    [self.addButtonScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.right.bottom.top.mas_equalTo(self.scrollView);
    }];
    
    [self createScrollView:self.addButtonScrollView];
    
    return self;
}

#pragma mark - 配置新增scrollView
-(void)createScrollView:(UIScrollView *)scrollView{
    
    int imageCount = 10;
    int pageCount = imageCount/8;
    int leftNum = imageCount%8;
    if(leftNum>0){
        pageCount ++;
    }
    ...
    [self.addButtonScrollView addSubview:view];
    
    [view mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.top.bottom.mas_equalTo(self.scrollView);
        make.size.mas_equalTo(self.addButtonScrollView.contentSize);
    }];
    ...
    [bgView1 mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(self.addButtonScrollView.mas_width).multipliedBy(1);
...
- (void)setTextView:(id<UITextInput>)textView {
    ...
    if (textView == nil) {
        if ([_textView isKindOfClass:[UITextView class]]) {
            ...
            [(UITextField *)_textView setInputAccessoryView:nil];
        }
    }
    else {
